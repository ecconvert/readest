...
// it engages on touch/pen, otherwise it eats the single tap / swipe that the
// reader uses to turn the page (the Apple Pencil page-jump/freeze regression).
const HOLD_MS = 300;
...
describe('useTextSelector instant-highlight still-hold gate', () => {
  ...
  test('a pen still hold also starts instant annotating after the hold elapses', async () => {
    const { result } = setup();

    result.current.handlePointerDown(doc, 0, pointerEvent('pen', 100, 100));
    await vi.advanceTimersByTimeAsync(HOLD_MS + 20);
    expect(result.current.isInstantAnnotating.current).toBe(true);
  });
  ...
  test('a pen swipe before the hold cancels arming (the Apple Pencil page-jump case)', async () => {
    const { result } = setup();

    result.current.handlePointerDown(doc, 0, pointerEvent('pen', 100, 100));
    result.current.handlePointerMove(doc, 0, { clientX: 175, clientY: 102 } as PointerEvent);
